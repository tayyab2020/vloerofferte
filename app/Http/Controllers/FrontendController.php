...
class FrontendController extends Controller
{
    ...
    public function products(Request $request)
    {
        ...
        if($category)
        {
            ...
            if(!$range_s && !$range_e)
            {
                $range_s = $all_products->where('product_models.estimated_price','>=',0)->select('product_models.estimated_price')->get()->min('estimated_price');
                $range_e = $all_products->where('product_models.estimated_price','>=',0)->select('product_models.estimated_price')->get()->max('estimated_price');
